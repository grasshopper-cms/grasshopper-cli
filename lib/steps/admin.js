...
module.exports = function(config){
    var adminConfig = {
        ...
        questions = [{
            ...
            type: 'input',
            name: 'base',
            message: 'What is your href attribute for the base tag in index.html?',
            default: adminConfig.base
        }],
        deferred = q.defer();
    ...
    function baseConfig(){
        inquirer.prompt(questions, function( answers ) {
            ...
            if(process.platform === 'win32'){
                answers.buildDirectory = answers.buildDirectory.replace(/\\/g,'\\\\');
            }

            utilsFs.directoryExistsAndWritable(answers.buildDirectory).then(
                function(){
                    writeConfig(config)
                        //.then(writeConfig)
                        .then(install)
